...
export interface ApiListProps {
    ...
    onUpdateApi: (id: string, updates: Partial<Omit<API, 'id' | 'projectId' | 'createdAt'>>) => void;
...
const ApiList = ({ apis, onCreateApi, onUpdateApi, onDeleteApi }: ApiListProps) => {
    const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
    const [editingApi, setEditingApi] = useState<API | null>(null);
    const [expandedApiId, setExpandedApiId] = useState<string | null>(null);

    const handleCreateApi = (apiData: Omit<API, 'id' | 'projectId' | 'createdAt'>) => {
        onCreateApi(apiData);
        setIsCreateModalOpen(false);
    };

    const handleUpdateApi = (apiData: Omit<API, 'id' | 'projectId' | 'createdAt'>) => {
        if (editingApi) {
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-dark-900 dark:text-dark-100">APIs</h2>
                    ...
                    return (
                        <Card key={api.id} className="p-4">
                            <div className="flex items-start justify-between">
                                <div className="flex-1 cursor-pointer" onClick={() => toggleExpand(api.id)}>
                                    <div className="flex items-center gap-3 mb-2">
                                        <span className={cn('px-2 py-1 rounded text-xs font-semibold', methodColors[api.method])}>
                                            {api.method}
                                        </span>
                            ...
                            {isExpanded && (
                                <div className="mt-4 pt-4 border-t border-dark-200 dark:border-dark-700 space-y-3">
                                    {api.headers.length > 0 && (
                                        <div>
                                            <h4 className="text-sm font-semibold text-dark-700 dark:text-dark-300 mb-2">Headers:</h4>
                                            <div className="space-y-1">
                                                {api.headers.map((header, index) => (
                                                    <div key={index} className="text-sm font-mono bg-dark-50 dark:bg-dark-900 p-2 rounded">
                                                        <span className="text-primary-600 dark:text-primary-400">{header.key}</span>
                                                        {': '}
                                                        <span className="text-dark-700 dark:text-dark-300">{header.value}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    {api.body && (
                                        <div>
                                            <h4 className="text-sm font-semibold text-dark-700 dark:text-dark-300 mb-2">Request Body:</h4>
